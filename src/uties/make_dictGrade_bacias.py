...
shp_bacias = ee.FeatureCollection(asset_bacias)
...
dictGrades = {}
for cc, nbacias in enumerate(listaNameBacias[:]):
    print(f" #{cc} processing bacia {nbacias}")
    shptmp = shp_bacias.filter(ee.Filter.eq('nunivotto4', nbacias))
    shp_gradestmp = shp_grid.filterBounds(shptmp.geometry())
    lstCod = shp_gradestmp.reduceColumns(ee.Reducer.toList(), ['indice']).get('list').getInfo()
    print(lstCod)
    dictGrades[nbacias] = lstCod


for kkey, lstVal in dictGrades.items():
    print(f"'{kkey}': {lstVal}")


with open('dict_basin_49_lista_grades.json', 'w') as arquivo_json:
    json.dump(dictGrades, arquivo_json, indent=4)
